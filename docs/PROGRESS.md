...
### `[x]` Task 21 — Reviews with moderation

- [x] Verified-purchase reviews; pending by default
- [x] Author sees own pending review; public sees approved only
- [x] Text stored raw, **escaped at render**, never `dangerouslySetInnerHTML`
- [x] Rating aggregates maintained by a Function

Most of the review surface was already scaffolded before this task — the `ReviewDoc` schema, the
`reviewStatusMachine`, the three review events with their notification routes, the four read
repositories, the `reviews` security rule and its five composite indexes. What Task 21 added is the
write side and the two UIs that drive it: submission, moderation, the rating trigger, and the
storefront and backoffice screens.

**Configuration.**

|                                   |                                                                                                 |
| --------------------------------- | ----------------------------------------------------------------------------------------------- |
| `content.product.reviews.*`       | New copy block (both stores): heading, empty state, verified label, form labels, pending notice |
| `RATE_LIMITS.review`              | 5/day per customer — a review is a rare, deliberate act; a flood is abuse                       |
| `RATE_LIMITS.adminCatalogueWrite` | 240/min — moderation reuses the catalogue-write bucket, not a new key                           |
| `reviews` indexes / rule          | Unchanged — the earlier scaffold's five indexes and API-only write rule hold                    |

**Demo.**

| Claim                                    | How to see it                                                                        |
| ---------------------------------------- | ------------------------------------------------------------------------------------ |
| A review is held until a moderator acts  | Submit on a PDP → the pending notice; the review is not in the public list           |
| The author sees their own pending review | `/account/reviews` shows it as "Awaiting approval", never the rejection reason       |
| The public sees only approved reviews    | `listPublishedReviews` and the PDP list filter on `published` (the shared constant)  |
| Verified purchase is a checked claim     | The badge appears only when a paid order of the customer's contains the product      |
| Review text is escaped, never injected   | A body of `<script>…</script>` renders as characters; asserted in three test suites  |
| The rating average is maintained         | Publishing a review moves `products.ratingAvg`/`ratingCount`; the trigger owns it    |
| Moderation is attributable and audited   | Each publish/reject records the moderator and appends `review.published`/`.rejected` |

**Notes.**

- **The rating aggregate is a recompute, not a counter.** `categoryProductCounter` maintains
  `categories.productCount` with `FieldValue.increment`, but a mean cannot be incremented. So the
  `reviews/{id}` trigger reads the product inside a transaction, applies a pure delta (count and a
  running sum of stars) computed in `@romp/core`, and writes the recomputed pair back. That also makes
  it safe under at-least-once delivery: a retried invocation recomputes from the current stored pair
  rather than double-applying, where an increment would drift. The pure arithmetic reconstructs the
  sum as `round(ratingAvg × ratingCount)` — the stored average is one decimal, so rounding recovers
  the integer sum of whole-star ratings — and floors the count at zero as a defensive guard.
- **The two audiences see different projections.** The public `PublicReviewView` carries no `userId`
  and no moderation trail; the author's `OwnReviewView` adds only the status, so a pending review
  reads as received; the moderator's `ModerationReviewView` adds the uid and the verified-purchase
  evidence. The rejection _reason_ is on no view returned to a customer — it is a moderation note,
  recorded for the audit and never surfaced, so a rejection does not invite an argument.
- **The verified-purchase badge is a factual claim, so it is derived server-side.** Submission reads
  the customer's own paid orders and matches the product against their line items in memory (Firestore
  cannot query into the immutable `items` array); the `ReviewDoc` schema then holds `verifiedPurchase`
  and the evidencing `orderId` consistent. A client cannot assert the badge.
- **Escaped at render is enforced at every render, not once.** The storefront PDP list, the account
  "your reviews" list, and the backoffice moderation queue all place the raw body as a plain JSX
  child — React escapes it — and a test in each suite feeds a `<script>` body and asserts no element
  is created. `dangerouslySetInnerHTML` appears nowhere near a review.
- **One review per product per customer, but a rejected one frees the slot.** The slot check counts
  only `pending` and `published` reviews, so a customer whose review was rejected may write another —
  which is the counterpart to not telling them why the first was rejected.
...
| 2026-09-11 | E2E  | The browser Firebase SDK is emulator-aware behind an opt-in flag, wired in a pure helper                                  | The server data layer already routed to the emulators via host vars; the browser SDK did not, so customer/admin login and the realtime bell could not run offline. `NEXT_PUBLIC_USE_FIREBASE_EMULATOR=true` now drives `connect*Emulator` in both apps, off by default so production is untouched. The decision and the connect calls live in a Firebase-free `firebase-emulator.ts` per app, because `firebase-client.ts` aborts the jsdom worker on import and is coverage-excluded — so the testable logic had to sit outside it.                                                                                                                                      |
